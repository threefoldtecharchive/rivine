...
func RegisterWalletHTTPHandlers(router Router, cs modules.ConsensusSet, wallet modules.Wallet, chainCts types.ChainConstants, requiredPassword string) {
	if wallet == nil {
	...
	router.GET("/wallet/blockstakestats", RequirePasswordHandler(NewWalletBlockStakeStatsHandler(cs, wallet, chainCts.RootDepth), requiredPassword))
	...
	router.POST("/wallet/sweep/seed", RequirePasswordHandler(NewWalletSweepSeedHandler(wallet), requiredPassword))
	...
	router.POST("/wallet/changepassphrase", RequirePasswordHandler(NewWalletChangePassphraseHandler(wallet), requiredPassword))
...
func NewWalletBlockStakeStatsHandler(cs modules.ConsensusSet, wallet modules.Wallet, rootDepth types.Target) httprouter.Handle {
	return func(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
		...
		tabsDiff, err := modules.EstimatedActiveBlockStakesAt(cs, rootDepth, 0, 0)
		if err != nil {
			WriteError(w, Error{"error after call to /wallet/blockstakestat: " + err.Error()}, walletErrorToHTTPStatus(err))
			return
		}
...
// NewWalletSweepSeedHandler creates a handler to handle API calls to /wallet/sweep/seed.
func NewWalletSweepSeedHandler(wallet modules.Wallet) httprouter.Handle {
	return func(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
		var seed modules.Seed
		...
		coins, blockstakes, err := wallet.SweepSeed(seed)
		if err != nil {
...
// NewWalletChangePassphraseHandler creates a handler to handle API calls to /wallet/changepassphrase.
func NewWalletChangePassphraseHandler(wallet modules.Wallet) httprouter.Handle {
	return func(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
		...
		err := wallet.ChangeKey(key, newKey)
		if err == nil {
			WriteSuccess(w)
			return
		}
