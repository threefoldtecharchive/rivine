...
func createWalletCmd(cli *CommandLineClient) *WalletCommand {
	...
	will be sent to your wallet.`,
		...
		changePassphraseCmd = &cobra.Command{
			...
			Run:   Wrap(walletCmd.changePassphraseCmd),
		}
...
// sweepCmd sweeps coins and funds from a seed.
func (walletCmd *walletCmd) sweepCmd() {
	mnemonic, err := speakeasy.Ask("Seed Mnemonic: ")
	if err != nil {
		cli.DieWithError("Reading seed mnemonic failed:", err)
	}
	seed, err := modules.InitialSeedFromMnemonic(mnemonic)
	if err != nil {
		cli.DieWithError("Invalid seed mnemonic given:", err)
	}

	fmt.Println("Sweeping the given seed, this may take a couple of minutes...")

	var swept api.WalletSweepPOST
	err = walletCmd.cli.PostResp(
		"/wallet/sweep/seed", fmt.Sprintf("seed=%s", seed.String()), &swept)
	if err != nil {
		cli.DieWithError("Could not sweep seed:", err)
	}
	currencyConvertor := walletCmd.cli.CreateCurrencyConvertor()
	fmt.Printf("Swept %v and %v BS from seed.\n",
		currencyConvertor.ToCoinStringWithUnit(swept.Coins), swept.BlockStakes)
}
...
Unconfirmed Delta:   %v
BlockStakes:         %v BS
`, encStatus, status.Height, currencyConvertor.ToCoinStringWithUnit(status.ConfirmedCoinBalance),
...
// changePassphraseCmd changes the passphrase of the wallet.
func (walletCmd *walletCmd) changePassphraseCmd() {
	passphrase, err := speakeasy.Ask("Current wallet passphrase: ")
	if err != nil {
		cli.DieWithError("Reading current passphrase failed:", err)
	}
	newPassphrase, err := speakeasy.Ask("New wallet passphrase: ")
	if err != nil {
		cli.DieWithError("Reading new passphrase failed:", err)
	}
	if newPassphraseConfirmation, err := speakeasy.Ask("Confirm new wallet passphrase: "); err != nil {
		cli.DieWithError("Reading confirmation of new passphrase failed:", err)
	} else if newPassphraseConfirmation != newPassphrase {
		cli.Die("confirmation of new passphrase does not match the given new passphrase")
	}
	qs := fmt.Sprintf("passphrase=%s&newpassphrase=%s", passphrase, newPassphrase)
	err = walletCmd.cli.Post("/wallet/changepassphrase", qs)
	if err != nil {
		cli.DieWithError("Changing the passphrase failed:", err)
	}
