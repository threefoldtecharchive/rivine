...
// errors.
func postEncryptionTesting(w *Wallet, masterKey crypto.TwofishKey) {
...
func TestIntegrationPreEncryption(t *testing.T) {
	...
	w1, err := New(wt.cs, wt.tpool, filepath.Join(wt.persistDir, modules.WalletDir), bcInfo, chainCts)
	if err != nil {
		t.Fatal(err)
	}
...
func TestIntegrationUserSuppliedEncryption(t *testing.T) {
	...
	_, err = wt.wallet.Init(masterKey)
	if err != nil {
		t.Error(err)
	}
	postEncryptionTesting(wt.wallet, masterKey)
}
...
func TestIntegrationBlankEncryption(t *testing.T) {
	...
	postEncryptionTesting(wt.wallet, crypto.TwofishKey(crypto.HashObject(seed)))
}
*/
...
func TestLock(t *testing.T) {
	...
	// Lock the wallet.
	siacoinBalance, _, err := wt.wallet.ConfirmedBalance()
	if err != nil {
		t.Error(err)
	}
	err = wt.wallet.Lock()
	...
	siacoinBalance2, _, err := wt.wallet.ConfirmedBalance()
	if err != nil {
		t.Error(err)
	}
...
func TestInitFromSeedConcurrentUnlock(t *testing.T) {
	...
	origBal, _, err := wt.wallet.ConfirmedBalance()
	if err != nil {
		t.Fatal(err)
	}
	...
	newBal, _, err := w.ConfirmedBalance()
	if err != nil {
		t.Fatal(err)
	}
...
func TestReset(t *testing.T) {
	...
	_, err = wt.wallet.Init(originalKey)
	if err != nil {
		t.Fatal(err)
	}
	postEncryptionTesting(wt.wallet, originalKey)
