...
var (
	...
	// bucketCoinOutputs maps a CoinOutputID to its CoinOutput. Only
	// outputs that the wallet controls are stored. The wallet uses these
	// outputs to fund transactions.
	bucketCoinOutputs = []byte("bucketCoinOutputs")
	// bucketCoinOutputs maps a BlockStakeOutputID to its BlockStakeOutput. Only
	// outputs that the wallet controls are stored. The wallet uses these
	// outputs to fund transactions.
	bucketBlockStakeOutputs = []byte("bucketBlockStakeOutputs")
	// bucketMultisigCoinOutputs maps a CoinOutputID to its CoinOutput. Only
	// outputs that a multisig wallet (one of this wallet seed's is part of) controls are stored.
	// The wallet uses these to list and spend multisig coin outputs.
	bucketMultisigCoinOutputs = []byte("bucketMultisigCoinOutputs")
	// bucketMultisigCoinOutputs maps a BlockStakeOutputID to its BlockStakeOutput. Only
	// outputs that a multisig wallet (one of this wallet seed's is part of) controls are stored.
	// The wallet uses these to list and spend multisig block stake outputs.
	bucketMultisigBlockStakeOutputs = []byte("bucketMultisigBlockStakeOutputs")
	...
	// bucketUnspentBlockStakeOutputs contains the unspent block stake outputs.
	bucketUnspentBlockStakeOutputs = []byte("bucketUnspentBlockStakeOutputs")
	...
	dbBuckets = [][]byte{
		...
		bucketMultisigCoinOutputs,
		bucketMultisigBlockStakeOutputs,
		bucketSpentOutputs,
		bucketWallet,
		bucketUnspentBlockStakeOutputs,
	}
...
func dbPutCoinOutput(tx *bolt.Tx, id types.CoinOutputID, output types.CoinOutput) error {
	return dbPut(tx.Bucket(bucketCoinOutputs), id, output)
}
func dbGetCoinOutput(tx *bolt.Tx, id types.CoinOutputID) (output types.CoinOutput, err error) {
	err = dbGet(tx.Bucket(bucketCoinOutputs), id, &output)
	return
}
func dbDeleteCoinOutput(tx *bolt.Tx, id types.CoinOutputID) error {
	return dbDelete(tx.Bucket(bucketCoinOutputs), id)
}
func dbForEachCoinOutput(tx *bolt.Tx, fn func(types.CoinOutputID, types.CoinOutput)) error {
	return dbForEach(tx.Bucket(bucketCoinOutputs), fn)
}

func dbPutBlockStakeOutput(tx *bolt.Tx, id types.BlockStakeOutputID, output types.BlockStakeOutput) error {
	return dbPut(tx.Bucket(bucketBlockStakeOutputs), id, output)
}
func dbGetBlockStakeOutput(tx *bolt.Tx, id types.BlockStakeOutputID) (output types.BlockStakeOutput, err error) {
	err = dbGet(tx.Bucket(bucketBlockStakeOutputs), id, &output)
	return
}
func dbDeleteBlockStakeOutput(tx *bolt.Tx, id types.BlockStakeOutputID) error {
	return dbDelete(tx.Bucket(bucketBlockStakeOutputs), id)
}
func dbForEachBlockStakeOutput(tx *bolt.Tx, fn func(types.BlockStakeOutputID, types.BlockStakeOutput)) error {
	return dbForEach(tx.Bucket(bucketBlockStakeOutputs), fn)
}

func dbPutMultisigCoinOutput(tx *bolt.Tx, id types.CoinOutputID, output types.CoinOutput) error {
	return dbPut(tx.Bucket(bucketMultisigCoinOutputs), id, output)
}
func dbGetMultisigCoinOutput(tx *bolt.Tx, id types.CoinOutputID) (output types.CoinOutput, err error) {
	err = dbGet(tx.Bucket(bucketMultisigCoinOutputs), id, &output)
	return
}
func dbDeleteMultisigCoinOutput(tx *bolt.Tx, id types.CoinOutputID) error {
	return dbDelete(tx.Bucket(bucketMultisigCoinOutputs), id)
}
func dbForEachMultisigCoinOutput(tx *bolt.Tx, fn func(types.CoinOutputID, types.CoinOutput)) error {
	return dbForEach(tx.Bucket(bucketMultisigCoinOutputs), fn)
}

func dbPutMultisigBlockStakeOutput(tx *bolt.Tx, id types.BlockStakeOutputID, output types.BlockStakeOutput) error {
	return dbPut(tx.Bucket(bucketMultisigBlockStakeOutputs), id, output)
}
func dbGetMultisigBlockStakeOutput(tx *bolt.Tx, id types.BlockStakeOutputID) (output types.BlockStakeOutput, err error) {
	err = dbGet(tx.Bucket(bucketMultisigBlockStakeOutputs), id, &output)
	return
}
func dbDeleteMultisigBlockStakeOutput(tx *bolt.Tx, id types.BlockStakeOutputID) error {
	return dbDelete(tx.Bucket(bucketMultisigBlockStakeOutputs), id)
}
func dbForEachMultisigBlockStakeOutput(tx *bolt.Tx, fn func(types.BlockStakeOutputID, types.BlockStakeOutput)) error {
	return dbForEach(tx.Bucket(bucketMultisigBlockStakeOutputs), fn)
}
