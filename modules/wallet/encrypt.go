...
// Init will create a primary seed for the wallet and encrypt it using
...
//
// Init can only be called once throughout the life of the wallet, and will
...
// or deleted.
func (w *Wallet) Init(masterKey crypto.TwofishKey) (modules.Seed, error) {
	if err := w.tg.Add(); err != nil {
		return modules.Seed{}, err
	}
	defer w.tg.Done()
