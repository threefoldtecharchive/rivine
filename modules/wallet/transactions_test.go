...
// 		t.Fatal(err)
// 	}
// 	if len(txns) != int(chainCts.MaturityDelay+1) {
...
// 		t.Fatal(err)
// 	}
// 	cs.addTransactionAsBlock(uc, sentValue.Add(wt.wallet.chainCts.MinimumTransactionFee))
// 	_, err = wt.wallet.SendCoins(sentValue, types.NewCondition(&types.NilCondition{}), nil)
// 	if err != nil {
// 		t.Fatal(err)
// 	}
...
// 		t.Fatal(err)
// 	}
// 	if len(txns) != int(chainCts.MaturityDelay+2+2) {
// 		t.Errorf("unexpected transaction history length: expected %v, got %v", chainCts.MaturityDelay+2+2, len(txns))
// 	}
//
// 	// Try getting a partial history for just the previous block.
// 	txns, err = wt.wallet.Transactions(chainCts.MaturityDelay+2, chainCts.MaturityDelay+2)
// 	if err != nil {
// 		t.Fatal(err)
// 	}
...
// 	sentValue := types.NewCurrency64(5000)
// 	sendTxn, err := wt.wallet.SendCoins(sentValue, types.NewCondition(&types.NilCondition{}), nil)
// 	if err != nil {
// 		t.Fatal(err)
// 	}
//
...
// 	*/
//
// 	txn, exists, err := wt.wallet.Transaction(sendTxn.ID())
// 	if err != nil {
// 		t.Fatal(err)
// 	}
// 	if !exists {
...
// 	}
// 	if txn.TransactionID != sendTxn.ID() {
...
// 	sendTxn, err = wt.wallet.SendBlockStakes(sentValue, types.NewCondition(&types.NilCondition{}))
// 	if err != nil {
// 		t.Fatal(err)
// 	}
// 	/*
...
//
// 	// Restart wallet
// 	wallet, err := New(wt.cs, wt.tpool, filepath.Join(wt.persistDir, modules.WalletDir), bcInfo, chainCts)
// 	if err != nil {
...
// 		t.Fatal(err)
// 	}
// 	_, err = wt.wallet.SendCoins(types.NewCurrency64(5005), types.NewCondition(types.NewUnlockHashCondition(addr)), nil)
// 	if err != nil {
// 		t.Fatal(err)
// 	}
//
...
// 				}
// 			}
// 			if output.FundType == types.SpecifierCoinOutput {
// 				if output.ID != types.OutputID(txn.Transaction.CoinOutputID(uint64(i))) {
// 					t.Fatal("coin output had incorrect output ID")
// 				}
// 			}
// 		}
...
// 	searchAddr := types.NewUnlockHash(types.UnlockTypePubKey, crypto.Hash{})
// 	err = dbAppendProcessedTransaction(wt.wallet.dbTx, modules.ProcessedTransaction{
// 		TransactionID: types.TransactionID{1},
