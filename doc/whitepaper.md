...
Additionally, there is complete separation of private and public key creation for greater security and convenience. In this model a server can be set up to only know the Master Public Key of a particular deterministic wallet. This allows the server to create as many public keys as is necessary for receiving funds, but a compromise of the MPK will not allow an attacker to spend from the wallet.
...
In POBS, the variables hashed are chosen in way that there is no randomness and people participating don’t win anything by trying to manipulate them. Since the search space is limited to 1 calculation per unspent blockstake output per second, the processing power required is neglectable.
...
Unspent BlockStake Outputs not on index 0 (transferred blockstakes) need to mature for 144 blocks before they can participate in the POBS protocol. This removes the benefit for manipulating the UTXO index variable in the POBS hash and pre calculating better future blockstake creation chances.
...
Every transaction has the possibility to contain arbitrary data. No validation is performed on the data itself. The maximum size of this arbitrary data is 1KiB. The minimal transaction fee depends on the size of the transaction so adding arbitrary data increases the required fee.
...
Every full node has the capability to run a public gateway. It indexes the entire chain and provides a public API to access it. Next to this “explorer” api, it accepts entire transactions and puts them on the network.

#### Smart gateways

Having a fixed number of publicly available gateways is easy but not secure, dynamic nor fair. It would be better if anyone could host a public gateway and everyone is able to discover it. In order to keep this secure, the gateways create smart contracts with locked collateral. The incentive to host such a gateway is that they are able to charge for providing the information and for putting transactions on the network. Proof of malicious gateways would make them lose the collateral.
